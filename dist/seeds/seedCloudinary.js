...
const uploadImage = async (filename) => {
    try {
        const imagePath = path_1.default.join(process.cwd(), 'public', 'uploads', filename);
        console.log(`  📤 Uploading ${filename}...`);
        const result = await (0, cloudinary_1.uploadFileToCloudinary)(imagePath, {
...
const seedWithCloudinary = async () => {
    try {
        ...
        const deletedCount = await Product_1.Product.deleteMany({});
        console.log(`   Deleted ${deletedCount.deletedCount} existing products`);
        console.log('\n🔍 Checking categories...');
        let artworkCategory = await Category_1.Category.findOne({ slug: 'artwork' });
        let shoesCategory = await Category_1.Category.findOne({ slug: 'shoes' });
        if (!artworkCategory) {
            artworkCategory = await Category_1.Category.create({
                name: 'Artwork',
                slug: 'artwork',
        ...
        if (!shoesCategory) {
            shoesCategory = await Category_1.Category.create({
                name: 'Shoes',
                slug: 'shoes',
        ...
        await Product_1.Product.insertMany(artworkProducts);
        ...
        await Product_1.Product.insertMany(shoeProducts);
